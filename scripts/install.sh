...
echo "test: $CODEX_BIN_INSTALL_PATH/codexcli"
echo "test2: $CODEX_CLI_PATH $CODEX_BIN_INSTALL_PATH/codexcli"

echo "[CODEX_CLI][INSTALL]: Install codexcli..."
sudo rm -rf $CODEX_BIN_INSTALL_PATH/codexcli
sudo ln -s $CODEX_CLI_PATH $CODEX_BIN_INSTALL_PATH/codexcli

if [ "$1" != "-i" ] && [ "$1" != "--image" ]; then
    echo "[CODEX_CLI][INSTALL]: Build codex image?"
    echo "If you also want to build and cache the newest codex image enter 'y'."
    read INPUT_VALUE
